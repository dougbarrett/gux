...
func main() {
	...
	code := generateClient(interfaces)
...
{{- if $method.HasReturn}}
func (c *{{$iface.ClientName}}) {{$method.Name}}(ctx context.Context{{range $method.PathParams}}, {{.}} int{{end}}{{if $method.HasBody}}, {{$method.BodyParam}} {{$method.BodyType}}{{end}}) ({{if $method.IsPointer}}*{{end}}{{if $method.IsSlice}}[]{{end}}{{$method.ReturnType | stripPrefix}}, error) {
	{{- if $method.IsPointer}}
	result, err := doRequest[{{$method.ReturnType}}](ctx, c.cfg, "{{$method.HTTPMethod}}", {{buildPath $method.Path}}{{if $method.HasBody}}, {{$method.BodyParam}}{{else}}, nil{{end}})
	if err != nil {
		return nil, err
	}
	return &result, nil
	{{- else}}
	return doRequest[{{if $method.IsSlice}}[]{{end}}{{$method.ReturnType | stripPrefix}}](ctx, c.cfg, "{{$method.HTTPMethod}}", {{buildPath $method.Path}}{{if $method.HasBody}}, {{$method.BodyParam}}{{else}}, nil{{end}})
	{{- end}}
}
{{- else}}
func (c *{{$iface.ClientName}}) {{$method.Name}}(ctx context.Context{{range $method.PathParams}}, {{.}} int{{end}}) error {
	return doRequestNoResponse(ctx, c.cfg, "{{$method.HTTPMethod}}", {{buildPath $method.Path}})
}
{{- end}}
{{end}}
{{end}}
