...
func (p *Pagination) render() {
	...
	if p.props.ShowInfo && p.props.TotalItems > 0 {
		...
		info.Set("className", "text-sm text-secondary")
	...
	for _, page := range pages {
		if page == -1 {
			...
			ellipsis.Set("className", "px-2 text-tertiary")
...
func (p *Pagination) createNavButton(text, label string, enabled bool, onClick func()) js.Value {
	...
	if enabled {
		className += " border-default hover:surface-overlay text-secondary cursor-pointer"
	} else {
		className += " border-subtle text-disabled cursor-not-allowed"
	}
...
func (p *Pagination) createPageButton(page int) js.Value {
	...
	if isCurrent {
		...
		className += " hover:surface-overlay text-secondary cursor-pointer"
	}
