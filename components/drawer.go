...
func NewDrawer(props DrawerProps) *Drawer {
	...
	baseClass := "fixed surface-base shadow-xl z-50 transition-transform duration-300 ease-in-out flex flex-col"
	...
	if props.Title != "" || props.ShowClose {
		...
		header.Set("className", "flex items-center justify-between p-4 border-b border-subtle")
		...
		if props.ShowClose {
			...
			closeBtn.Set("className", "p-1 hover:surface-overlay rounded text-secondary text-xl")
