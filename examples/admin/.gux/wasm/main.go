...
func main() {
	...
	render = func() {
		...
		case "/activity":
			component = pages.Activity(router)
		case "/users/new":
			component = pages.UserNew(router)
		case "/users":
			component = pages.Users(router)
		case "/":
			component = pages.Dashboard(router)
		}
		if component == nil {
			if matchRoute(path, "/users/", "/edit") {
				router.SetRouteParams(map[string]string{"id": extractRouteParam(path, "/users/", "/edit")})
				component = pages.UserEdit(router)
			} else if matchRoute(path, "/users/", "") {
				router.SetRouteParams(map[string]string{"id": extractRouteParam(path, "/users/", "")})
				component = pages.UserDetail(router)
			}
		}
		if component == nil {
			component = pages.Dashboard(router)
