...
func main() {
	...
	render = func() {
		...
		switch path {
		case "/profile":
			component = pages.Profile(router)
		case "/settings":
			component = pages.Settings(router)
		case "/projects/new":
			component = pages.ProjectNew(router)
		case "/projects":
			component = pages.Projects(router)
		case "/":
			component = pages.Dashboard(router)
		}
		if component == nil {
			if matchRoute(path, "/projects/", "/edit") {
				router.SetRouteParams(map[string]string{"id": extractRouteParam(path, "/projects/", "/edit")})
				component = pages.ProjectEdit(router)
			} else if matchRoute(path, "/projects/", "") {
				router.SetRouteParams(map[string]string{"id": extractRouteParam(path, "/projects/", "")})
				component = pages.ProjectDetail(router)
			}
		}
		if component == nil {
			component = pages.Dashboard(router)
